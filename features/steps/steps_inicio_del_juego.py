...
def ingreso_al_modo_deseado(context,x):
    context.game._game_state = Game_State.CONFIG_GAME
    if x == "bacteriofagos":
...
def modo_de_juego(context,modo):
    if modo == "bacteriofagos":
        assert context.game._game_mode.__eq__(Game_Mode.BACTERIOPHAGE)
    else:
        assert context.game._game_mode.__eq__(Game_Mode.ANTIBIOTIC)
...
def completar_parametros(context,ac,a,b,c,d):
    if ac == "b":
        ...
        if ac == "a":
            ...
            context.game._cant_other = c
            context.game._frecuency_other = d
    context.game._game_state = Game_State.START_GAME
...
def tablero_parametrizado(context,ac,a,b,c,d):
    if ac == "b":
        ...
        if ac == "a":
            ...
            assert context.game._frecuency_other == d
        else:
            assert context.game._cant_other == c
            assert context.game._frecuency_other == d
